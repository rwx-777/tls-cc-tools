...
TLS_extensions *ssl_get_extension(unsigned short extension,
    TLS_handshake_1 *handshake);
unsigned char *ssl_cert_req_get_disting_names(
    TLS_handshake_generic *handshake);
char *get_disting_string(TLS_rdn_sequence *seq);
unsigned int get_ssl_3_byte_number(unsigned char *bytes);

void buffered_ssl_ctor(buffered_ssl_ctx *ctx);
int buffered_ssl_add_data(buffered_ssl_ctx *ctx, unsigned char *data,
    unsigned int length);
