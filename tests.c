...
unsigned char *
mod_server_cert_end(void *state, unsigned char *data, int len, int *out_len)
{
	TLS_record *record;
	TLS_handshake_generic *handshake;
	buffered_ssl_ctx *bssl = (buffered_ssl_ctx *)state;
		...
		if (record->record_type == TLS_RECORD_HANDSHAKE) {
			...
			if (handshake->handshake_type ==
				...
				if (total_length > 6) {
					...
					pos += 3;
					cert_length =
					    get_ssl_3_byte_number(pos);
					/* go to the last byte */
					pos += cert_length + 3 - 1;
					write_out(PRINT_INFO, "Changing last "
					    "byte of the first certificate "
					    "from 0x%02x to 0x%02x.", *pos,
					    (*pos) + 1);
...
unsigned char *
mod_server_cert_middle(void *state, unsigned char *data, int len, int *out_len)
{
	TLS_record *record;
	TLS_handshake_generic *handshake;
	...
	while ((db = buffered_ssl_get_record(&hstate->bssl))) {
		...
		if (record->record_type == TLS_RECORD_HANDSHAKE) {
			...
			if (handshake->handshake_type ==
				...
				if (total_length > 6) {
					pos = (unsigned char *)(handshake + 1);
					mod_pos = (unsigned char *)memmem(pos,
					    total_length, hstate->modulus,
					    hstate->size);

					if (mod_pos) {
						write_out(PRINT_INFO,
						    "Changing first byte of "
						    "certificate modulus from "
						    "0x%02x to 0x%02x.",
						    *mod_pos, (*mod_pos) + 1);
						/* last byte of data */
						++(*mod_pos);
...
int
FCS_TLSC_EXT_1_1_TEST_1_extra_ciphers(int ssock, SSL_CTX *ssl_ctx,
    ...
    ec_info *ec_list, int ec_count)
{
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx,
	    cipher_list[0].openssl_name))) {
		write_lower_level();
		return (0);
	}
	bio_ssl_read = BIO_new(BIO_s_mem());
	...
	do_handshake(rsock, ssl, bio_ssl_read, bio_ssl_write,
	    &get_cipher_suites, &hstate, 0, 0);
...
int
FCS_TLSC_EXT_1_1_TEST_1_test_connections(int ssock, SSL_CTX *ssl_ctx,
    cipher_suite_info *cipher_list, int cipher_count, int req_count)
{
	SSL *ssl;
	int ret = 1;
	...
	for (i = 0; i < cipher_count; ++i) {
		write_out(PRINT_OUTPUT, "Testing %s cipher suite %s.",
		    ((i < req_count) ? "mandatory" : "optional"),
		    cipher_list[i].std_name);

		write_raise_level();

		if (!(ssl = init_ssl_with_cipher(ssl_ctx,
		    cipher_list[i].openssl_name))) {
			if (i < req_count)
				ret = 0;
		} else if (!do_tls_connection(ssock, ssl, 0, 0, 0, 0, 1, 0)) {
			if (i < req_count)
				ret = 0;
			SSL_free(ssl);
		}
...
int
FCS_TLSC_EXT_1_1_TEST_1(int ssock, SSL_CTX *ssl_ctx,
    cipher_suite_info *cipher_list, int cipher_count, int req_count,
    ec_info *ec_list, int ec_count)
{
	write_raise_level();

	if (!FCS_TLSC_EXT_1_1_TEST_1_test_connections(ssock, ssl_ctx,
	    cipher_list, cipher_count, req_count)) {
		write_out(PRINT_OUTPUT, "Failed to establish required TLS "
		    "connection using a required crypto suite.");
		write_lower_level();
		return (0);
	}
	if (!FCS_TLSC_EXT_1_1_TEST_1_extra_ciphers(ssock, ssl_ctx, cipher_list,
	    cipher_count, req_count, ec_list, ec_count)) {
		write_out(PRINT_OUTPUT, "TLS connection supports unapproved "
		    "crypto suite(s) or elliptic curve.");
		write_lower_level();
...
int
FCS_TLSC_EXT_1_1_TEST_2(int ssock, SSL_CTX *ssl_ctx, X509 *ca_cert,
    ...
    const char *cipher_suite)
{
	...
	write_out(PRINT_INFO, "Generating %d bit RSA non-server authentication"
	    " cert.", bits);
	if (!create_rsa_cert_v3(bits, "USA", "QQQ", "IAD", "qqq.gov", rand(),
	    1, &rsa_cert, &rsa_priv_key)) {
		write_out(PRINT_ERROR, "Unable to generate non-server "
		    "authentication cert.");
		goto error_die;
	}
	...
	if (!sign_x509_cert(ca_cert, ca_private_key, rsa_cert, EVP_sha256())) {
		write_out(PRINT_ERROR, "Unable to sign non-server "
		    "authentication cert.");
		goto error_die;
	}
	...
	if (!SSL_check_private_key(ssl)) {
		write_out(PRINT_ERROR, "RSA Certificate and private key do "
		    "not match!");
...
int
FCS_TLSC_EXT_1_1_TEST_3_4(int ssock, SSL_CTX *ssl_ctx,
    const char *cipher_name, unsigned short alt_cipher_id)
{
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		...
		do_tls_connection(ssock, ssl, &mod_chosen_cipher_term,
		    &hstate, &mod_chosen_cipher, &hstate, 0, 0);
		ret = hstate.success;
		SSL_free(ssl);
	}
...
int
FCS_TLSC_EXT_1_1_TEST_5__1(int ssock, SSL_CTX *ssl_ctx,
    const char *cipher_name)
{
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		...
		do_tls_connection(ssock, ssl, &mod_server_version_term,
		    &hstate, &mod_server_version, &hstate, 0, 0);
		ret = hstate.success;
		SSL_free(ssl);
	}
...
int
FCS_TLSC_EXT_1_1_TEST_5__2(int ssock, SSL_CTX *ssl_ctx,
    const char *cipher_name)
{
	SSL *ssl;
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		...
		ret = do_tls_connection(ssock, ssl, 0, 0, &mod_server_nonce,
		    &bssl, 0, 0);
		SSL_free(ssl);
	}
...
int
FCS_TLSC_EXT_1_1_TEST_5__3(int ssock, SSL_CTX *ssl_ctx, int weak_list_count,
    cipher_suite_info *weak_list)
{
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, "ALL:COMPLEMENTOFALL")))
		...
		do_tls_connection(ssock, ssl, &change_client_ciphers, &hstate,
		    0, 0, 0, 0);
		ret = hstate.success;
...
int
FCS_TLSC_EXT_1_1_TEST_5__4(int ssock, SSL_CTX *ssl_ctx,
    const char *dh_cipher_name)
{
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, dh_cipher_name)))
		...
		do_tls_connection(ssock, ssl, &mod_server_key_exch_term,
		    &hstate, &mod_server_key_exch, &hstate, 0, 0);
		ret = hstate.success;
		SSL_free(ssl);
	}
...
int
FCS_TLSC_EXT_1_1_TEST_5__5(int ssock, SSL_CTX *ssl_ctx,
    const char *cipher_name)
{
	SSL *ssl;
	int ret;
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		...
		ret = do_tls_connection(ssock, ssl, 0, 0, &mod_server_fin,
		    &hstate, 0, 0);
		SSL_free(ssl);
	}
...
int
FCS_TLSC_EXT_1_1_TEST_5__6(int ssock, SSL_CTX *ssl_ctx,
    const char *cipher_name)
{
	SSL *ssl;
	int ret;
	...
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		...
		ret = do_tls_connection(ssock, ssl, 0, 0,
		    &mod_server_scram_next, &hstate, 0, 0);
		SSL_free(ssl);
	}
...
int
build_test_cert_chains_output(SSL_CTX **out_ssl_ctx, X509 **out_cert,
    EVP_PKEY **out_pk, X509 *root_cert, EVP_PKEY *root_pk, int depth, int bits,
    ...
    const char *common_name, const char *alt_name, int serial, int days_valid)
{
	...
	if (!build_rsa_x509_chain(depth, cert_stack, pk_stack, root_cert,
	    root_pk, bits, country, org_name, org_unit, common_name, alt_name,
	    serial, days_valid)) {
	...
	ssl_ctx = init_ssl_server_ctx(TLS_method(), cert_stack[depth - 1],
	    pk_stack[depth - 1], 0, 0, 0, 0, root_cert);
	if (!ssl_ctx) {
...
int
build_test_cert_chains_ex(SSL_CTX **out_ssl_ctx, X509 **out_cert,
    EVP_PKEY **out_pk, X509 *root_cert, EVP_PKEY *root_pk, int depth,
    int bits, const char *country, const char *org_name, const char *org_unit,
    const char *common_name, const char *alt_name, int serial, int days_valid,
    int set_ca, int set_server_auth)
{
	SSL_CTX *ssl_ctx = 0;
	X509 **cert_stack = 0;
	EVP_PKEY **pk_stack = 0;
	...
	/* build our chain of certificates */
	if (!build_rsa_x509_chain_ex(depth, cert_stack, pk_stack, root_cert,
	    root_pk, bits, country, org_name, org_unit, common_name, alt_name,
	    serial, days_valid, set_ca, set_server_auth)) {
		write_out(PRINT_ERROR, "Unable to generate certificate chain.");
		ret = 0;
		goto error_die;
	}
	...
	/*
	 * add the rest of our certificate chain this will cause SSL_CTX
	 * to take ownership of everything in cert_stack
	 */
...
int
build_test_cert_chains(SSL_CTX **out_ssl_ctx, X509 **out_cert,
    EVP_PKEY **out_pk, X509 *root_cert, EVP_PKEY *root_pk, int depth, int bits,
    const char *country, const char *org_name, const char *org_unit,
    const char *common_name, const char *alt_name, int serial, int days_valid)
{

	return (build_test_cert_chains_ex(out_ssl_ctx, out_cert, out_pk,
	    root_cert, root_pk, depth, bits, country, org_name, org_unit,
	    common_name, alt_name, serial, days_valid, SET_CA_TRUE, 1));
}

int
FCS_TLSC_EXT_1_2_TEST_1(int ssock, const char *cipher_name, int depth,
    ...
    const char *alt_name, int serial, int days_valid)
{
	...
	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    depth, bits, country, org_name, org_unit, cn_str, alt_str, serial,
	    days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
	} else if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name)))
		ret = 0;
	else {
		ret = do_tls_connection(ssock, ssl, 0, 0, 0, 0, 0, 0);
		SSL_free(ssl);
...
int
FCS_TLSC_EXT_1_2_TEST_2(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
	...
	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    depth, bits, country, org_name, org_unit, common_name, alt_str,
	    serial, days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
...
int
FCS_TLSC_EXT_1_2_TEST_3(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
	...
	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    depth, bits, country, org_name, org_unit, common_name, 0, serial,
	    days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
...
int
FCS_TLSC_EXT_1_2_TEST_4(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
	...
	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    depth, bits, country, org_name, org_unit, cn_str, alt_name,
	    serial, days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
...
int
FCS_TLSC_EXT_1_2_TEST_5__1(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
	...
	snprintf(wild_char_domain, CERT_NAME_SIZE, "%s.*.%s", WILD_START,
	    common_name);

	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    depth, bits, country, org_name, org_unit, wild_char_domain, 0,
	    serial, days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
...
int
FCS_TLSC_EXT_1_2_TEST_5__2a(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
...
int
FCS_TLSC_EXT_1_2_TEST_5__2b(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
...
int
FCS_TLSC_EXT_1_2_TEST_5__3(int ssock, const char *cipher_name, int depth,
    X509 *root_cert, EVP_PKEY *root_pk, int bits, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
...
int
FCS_TLSC_EXT_1_3_TEST_1a(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	SSL_CTX *ssl_ctx = 0;
	SSL *ssl;
	int ret = 0;
	...
	if (!build_test_cert_chains(&ssl_ctx, &cert, &pk, root_cert, root_pk,
	    3, bits, country, org_name, org_unit, common_name, 0, serial,
	    days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
...
int
FCS_TLSC_EXT_1_3_TEST_1b(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
...
int
FCS_TLSC_EXT_1_4_TEST_1(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!generate_rsa_client_cert(bits, CLIENT_COUNTRY, CLIENT_ORG,
	    CLIENT_UNIT, CLIENT_NAME, rand(), 1, root_cert, root_pk,
	    &client_cert, &client_pk, EVP_sha256())) {
		ret = 0;
		goto error_die;
	}
	write_out(PRINT_OUTPUT, "Use %s or %s as client authentication certs.",
	    CLIENT_CERT, CLIENT_CERT_P12);
	export_cert_to_pem(client_cert, CLIENT_CERT, 0);
	export_pk_to_pem(client_pk, CLIENT_CERT, 1);
	export_key_pair_to_pkcs12(client_cert, client_pk, CLIENT_CERT_P12,
	    CLIENT_P12_PWD, CLIENT_P12_NAME);
	...
	if (!generate_rsa_ca(FAKE_BITS, FAKE_COUNTRY, FAKE_ORG, FAKE_UNIT,
	    FAKE_CA, rand(), 1, &fake_cert, &fake_pk, EVP_sha256())) {
		ret = 0;
		goto error_die;
	}
	if (!(ssl = init_ssl_with_cipher(ssl_ctx, cipher_name))) {
	...
	/*
	 * SSL_set_verify(ssl,
	 * 	SSL_VERIFY_FAIL_IF_NO_PEER_CERT | SSL_VERIFY_PEER, 0);
	 */
	...
	ret = do_tls_connection(ssock, ssl, 0, 0, &mod_client_cert_request,
	    &bssl, 0, 1);
...
int
FCS_TLSC_EXT_1_5_TEST_1(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, const char *ec_curve, const char *country,
    const char *org_name, const char *org_unit, const char *common_name,
    const char *alt_name, int serial, int days_valid)
{
	SSL_CTX *ssl_ctx = 0;
	...
	ssl_ctx = init_ssl_server_ctx(TLS_method(), 0, 0, 0, ec_curve,
	    ecdsa_cert, ecdsa_priv_key, root_cert);
...
int
FCS_TLSC_EXT_1_6_TEST_1(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!create_rsa_cert_v3(bits, country, org_name, org_unit, common_name,
	    rand(), 1, &rsa_cert, &rsa_priv_key)) {
	...
	ssl_ctx = init_ssl_server_ctx(TLS_method(), rsa_cert, rsa_priv_key, 0,
	    0, 0, 0, root_cert);
...
int
FIA_X509_EXT_1_1_TEST_1(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!build_test_cert_chains_output(&ssl_ctx, &cert, &pk, root_cert,
	    root_pk, 3, bits, country, org_name, org_unit, common_name, 0,
	    serial, days_valid)) {
		cert = 0;
		pk = 0;
		ret = 0;
	...
	write_out(PRINT_OUTPUT, "Ensure a valid certification path DOES NOT"
	    " exist before connecting.");
	if (!(ret = do_tls_connection(ssock, ssl, 0, 0, 0, 0, 0, 0)))
		goto error_die;
	write_out(PRINT_OUTPUT, "Add the %s*.pem certificates to create a "
	    "valid validation path before connecting.", INTER_CERT);
	if (!(ret = do_tls_connection(ssock, ssl, 0, 0, 0, 0, 1, 0)))
		goto error_die;
	write_out(PRINT_OUTPUT, "Remove one of the %s*.pem certificates from"
	    " the validation path.", INTER_CERT);
	if (!(ret = do_tls_connection(ssock, ssl, 0, 0, 0, 0, 0, 0)))
		goto error_die;
...
int
FIA_X509_EXT_1_1_TEST_2(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!create_rsa_cert_v3_ex(bits, country, org_name, org_unit,
	    common_name, serial, -60 * 60 * 24 * 2, -60 * 60 * 24 * 1, &cert,
	    &pk)) {
		ret = 0;
		goto error_die;
	}
	...
	ssl_ctx = init_ssl_server_ctx(TLS_method(), cert, pk, 0, 0, 0, 0,
	    root_cert);
	if (!ssl_ctx) {
		write_out(PRINT_ERROR, "Error creating SSL context.");
...
int
FIA_X509_EXT_1_1_TEST_5(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!build_rsa_x509_chain(1, &cert, &pk, root_cert, root_pk, bits,
	    country, org_name, org_unit, common_name, alt_name, serial,
	    days_valid)) {
	...
	ret = do_tls_connection(ssock, ssl, 0, 0, &mod_server_cert_start,
	    &bssl, 0, 0);
...
int
FIA_X509_EXT_1_1_TEST_6(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	ret = do_tls_connection(ssock, ssl, 0, 0, &mod_server_cert_end, &bssl,
	    0, 0);
...
int
FIA_X509_EXT_1_1_TEST_7(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	ret = do_tls_connection(ssock, ssl, 0, 0, &mod_server_cert_middle,
	    &hstate, 0, 0);
...
int
FIA_X509_EXT_1_2_TEST_1(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!build_test_cert_chains_ex(&ssl_ctx, &cert, &pk, root_cert,
	    root_pk, 2, bits, country, org_name, org_unit, common_name,
	    alt_name, serial, days_valid, SET_CA_NONE, 1)) {
...
int
FIA_X509_EXT_1_2_TEST_2(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!build_test_cert_chains_ex(&ssl_ctx, &cert, &pk, root_cert,
	    root_pk, 2, bits, country, org_name, org_unit, common_name,
	    alt_name, serial, days_valid, SET_CA_FALSE, 1)) {
		write_out(PRINT_ERROR, "Unable to generate certificates.");
		ret = 0;
		goto error_die;
	}
...
int
FIA_X509_EXT_1_2_TEST_3(int ssock, const char *cipher_name, X509 *root_cert,
    EVP_PKEY *root_pk, int bits, const char *country, const char *org_name,
    const char *org_unit, const char *common_name, const char *alt_name,
    int serial, int days_valid)
{
	...
	if (!build_test_cert_chains_ex(&ssl_ctx, &cert, &pk, root_cert,
	    root_pk, 2, bits, country, org_name, org_unit, common_name,
	    alt_name, serial, days_valid, SET_CA_TRUE, 1)) {
		write_out(PRINT_ERROR, "Unable to generate certificates.");
		ret = 0;
		goto error_die;
	}
