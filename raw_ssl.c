...
unsigned short *
ssl_get_cipher_suites(TLS_handshake_1 *handshake)
{

	return (unsigned short *)(((unsigned char *)(handshake + 1))
	    + handshake->session_length);
}

TLS_extensions *
ssl_get_extension(unsigned short extension, TLS_handshake_1 *handshake)
{
	unsigned short *cipher_suite_len = ssl_get_cipher_suites(handshake);
	unsigned char *compression_len = (((unsigned char *)(cipher_suite_len
	    + 1)) + ntohs(*cipher_suite_len));
	unsigned short *extension_len = (unsigned short *)(compression_len
	    + *compression_len + 1);
	...
	for (i = 0; i < ntohs(*extension_len); i += length) {
		if (ntohs(extension_data->type) != extension) {
			length = sizeof (TLS_extensions) +
			    ntohs(extension_data->total_length);
			extension_data = (TLS_extensions *)
			    (((unsigned char *)extension_data) + length);
		} else
...
unsigned char *
ssl_cert_req_get_disting_names(TLS_handshake_generic *handshake)
{
	unsigned char *cert_type_count = (unsigned char *)(handshake + 1);
	unsigned short *sig_hash_algos = (unsigned short *)(cert_type_count
	    + *cert_type_count + 1);
	unsigned short *disting_len = (unsigned short *)
	    (((unsigned char *)(sig_hash_algos + 1)) + ntohs(*sig_hash_algos));

	return ((unsigned char *)disting_len);
}

/* free returned memory */
char *
get_disting_string(TLS_rdn_sequence *seq)
{
	char *str;
...
unsigned int
get_ssl_3_byte_number(unsigned char *bytes)
{
...
int
buffered_ssl_expand_buffer(buffered_ssl_ctx *ctx, int size)
{
	unsigned int rem;
...
void
buffered_ssl_ctor(buffered_ssl_ctx *ctx)
{
...
int
buffered_ssl_add_data(buffered_ssl_ctx *ctx, unsigned char *data,
    unsigned int length)
{
	...
	if (ctx->data_length > ctx->buffer_size) {
		write_out(PRINT_ERROR, "CRITICAL ERROR: Buffer overflow while"
		    " buffering TLS data.");
		return (0);
	}
	return (1);
...
data_buffer *
buffered_ssl_get_record(buffered_ssl_ctx *ctx)
{
	TLS_record *record;
	data_buffer *data;
	...
	/* the size excludes the actual header itself: */
	total_size = record_len + sizeof (TLS_record);
...
void
buffered_ssl_dtor(buffered_ssl_ctx *ctx)
{
...
void
data_buffer_ctor(data_buffer *db)
{
...
int
data_buffer_merge(data_buffer *dest, data_buffer *src)
{
...
void
data_buffer_dtor(data_buffer *db)
{
	if (db->data)
		free(db->data);
	db->length = 0;
